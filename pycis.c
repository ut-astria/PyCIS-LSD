...
static PyObject * pycis(PyObject * self, PyObject * args)
{
  ...
  */
  //printf("LOAING SCALE/SIG = %.2f / %.2f\n",inputv[5],inputv[6]); fflush(stdout);
  if(markovOnly==0)
  {
    ...
    if (inputv_size>16)
    {
      printf("PKernel: (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f)\n",
        inputv[10],inputv[11], inputv[12],inputv[13],inputv[14],inputv[15]);
      printf("P3DKern: (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f)\n",
        inputv[16],inputv[17], inputv[18],inputv[19],inputv[20],inputv[21]);
      printf("OKernel: (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f)\n",
        inputvorth[10],inputvorth[11], inputvorth[12],
        inputvorth[13],inputvorth[14],inputvorth[15]);
      printf("O3DKern: (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f)\n",
        inputvorth[16],inputvorth[17], inputvorth[18],
        inputvorth[19],inputvorth[20],inputvorth[21]);    
    }
    else
    {
      printf("PKernel: (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f)\n",
    ...
    */
    ...
    else
    {
      ...
      else out = lsd3centerb(&n,image,X,Y,Z,image0,X0,Y0,Z0,inputv,inputv_size,inputvorth,0);
    } 
  }
  else if(markovOnly==1)
  {
    ...
    printf("Kernel: (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f)\n",
      inputv[10],inputv[11], inputv[12],inputv[13],inputv[14],inputv[15]);
    if (inputv_size>16)
      printf("3DKern: (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f), (p11=%.4f, p10=%.4f)\n",
        inputv[16],inputv[17], inputv[18],inputv[19],inputv[20],inputv[21]);
    fflush(stdout);
    */
  } 
  ...
  {
         out = lsd3bgrad(&n,image,X,Y,Z,inputv,inputv_size,inputvorth);
  }

  else if(markovOnly==-2)
  {
         printf("RETURNING REGION IMG");fflush(stdout);
         out = lsd3centerb(&n,image,X,Y,Z,image0,X0,Y0,Z0,inputv,inputv_size,inputvorth,1);

  }
  else if(markovOnly==-3)
  {
         printf("RETURNING RECT IMG");fflush(stdout);
         out = lsd3centerb(&n,image,X,Y,Z,image0,X0,Y0,Z0,inputv,inputv_size,inputvorth,2);
