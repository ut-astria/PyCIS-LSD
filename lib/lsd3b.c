...
lib/lsd3b.c: main pipline for 1st order line detection 
...
double * LineSegmentDetectionb( int * n_out,
                               ...
                               unsigned char ** used_img, double * output, double * output_2, double * output_4,double logNT)
{
  //initalization 
  image_double image;
...
//set size variables
  ...
  */
  /*
  if (inputv_size>13) 
    logNT = 5.0 * ( log10( (double) inputv[7] ) + log10( (double) inputv[8] )) / 2.0
          + log10(3.0 );    
  else
...
double * LineSegmentDetectionCenterb( int * n_out,
                               ...
                               double * outputP, double * outputP_2, double * outputP_4, ntuple_list out, int k,double logNT)
{
  //initalization 
  image_double image;
...
//set size variables
  ...
  */
  ...
  //INSTANTIATION
  int lx,ly,lz,lx2,ly2,lz2,lxm,lym,lzm;
  int lxx,lyy,lzz;
  int searchbox=3;
  ...
  { 
    ...
    lxm = (int) floor((lx+lx2)/2);
    lym = (int) floor((ly+ly2)/2);
    if ((lz2!=k) || (lz!=k)) continue;
    //use midpoint if possible, resort to an endpoint, or skip
    if ( (lxm<=0.)||(lym<=0.)||(lxm>=xsize)||(lym>=ysize))
    {
      //printf("ERROR: 2Dlsd3b: midpoint out of bounds");fflush(stdout);
      continue; 
      if ( (lx<=0.)||(ly<=0.)||(lx>=xsize)||(ly>=ysize))
      {
        if ( (lx2<=0.)||(ly2<=0.)||(lx2>=xsize)||(ly2>=ysize))
        {
          continue;
        } //if both points outside valid domain, skip
        else {lxm=lx2; lym=ly2;}
      }
      else {lxm=lx; lym=ly;}
    }
    lstheta = line_angle(lx,ly,lx2,ly2);    
    lxx=lxm; lyy=lym; lzz=lz;
                               ...
                               double * inputv, double inputv_size, double * inputvorth)
{
  ...
  */
  ...
  if (inputv_size>13) 
    logNT = 8.0 * ( log10( (double) inputv[7] ) + log10( (double) inputv[8] ) + log10((double) inputv[9])) / 3.0
          + log10(3.0 );    
  else
  ...
  double logNT2;
  if (inputv_size>13) 
    logNT2 = 5.0 * ( log10( (double) inputv[7] ) + log10( (double) inputv[8] )) / 2.0
          + log10(3.0 );    
  else
    logNT2 = 5.0 * (log10((double)xsize) + log10((double)ysize)) / 2.0
          + log10(3.0);
  logNT2=logNT;
  ...
  */
  ...
  for(k=0; k<zsize; k++)
  {
    ...
    out_2d = LineSegmentDetectionb(&n_out_2d, imgsub, X, Y,
                          ...
                          output, output_2, output_4,logNT2);
                               ...
                               double * inputv, double inputv_size, double * inputvorth)
{
  ...
  double * return_value = (double *) malloc(xsize*ysize*zsize*sizeof(double)); 
  for(i=0;i<(int)(xsize*ysize*zsize);i++)
  {
   //printtest 
   if ((angles->az->data[i] != NOTDEF) &&  (angles->el->data[i] != NOTDEF))
        //return_value[i] = isaligned3ORTH(angles->az->data[i],angles->el->data[i],-M_PI/4.,2.*M_PI/3.,prec);//>=cos(prec);
        return_value[i] = (1.-fabs(cos(angles->el->data[i])));//>=cos(prec);
        //return_value[i] = modgrad->data[i];
    else
        return_value[i]=0.;
  }
                               ...
                               double * inputv, double inputv_size, double * inputvorth,int printreg)
{
  ...
  double * return_value;
  if(printreg>0) return_value = (double *) malloc(X*Y*Z*sizeof(double)); 
  grads angles;
  image3_double modgrad;
  image3_char used;
  struct coorlist3 * list_p;
  ...
  */
  ...
  int ic=0;
  //if (inputv_size>16) ic=6;
  p11=inputvorth[10+ic];  p01=inputvorth[11+ic];
  p1=p;  p0=1-p1;
  outputP[0]=p0; outputP[1]=p11; outputP[2]=p01;
  min_reg_size=(int) (-log_eps-logNT-log10(p1))/log10(p11)+1;
  NFA_matrix(output,p0,p11,p01,N);
  p11=inputvorth[12+ic];  p01=inputvorth[13+ic];
  p1=p/2;  p0=1-p1;
  outputP_2[0]=p0; outputP_2[1]=p11; outputP_2[2]=p01;
  min_reg_size_2=(int) (-log_eps-logNT-log10(p1))/log10(p11)+1; 
  NFA_matrix(output_2,p0,p11,p01,N);
  p11=inputvorth[14+ic];  p01=inputvorth[15+ic];
  p1=p/4;  p0=1-p1;
  outputP_4[0]=p0; outputP_4[1]=p11; outputP_4[2]=p01;
  min_reg_size_4=(int) (-log_eps-logNT-log10(p1))/log10(p11)+1;
  NFA_matrix(output_4,p0,p11,p01,N);
  ...
  */
  ...
  for(k=0; k<zsize; k++)
  {
    ...
    out_2d = LineSegmentDetectionCenterb(&n_out_2d, imgsub, X, Y,
                          ...
                          outputP, outputP_2, outputP_4, out, k,logNT2);
    //enter used data into main matrix for exclusion principle 
    for(i=0;i<xsize;i++)
        for(j=0;j<ysize;j++)
            used->data[k+zsize*(i+j*xsize)] = used_img[i+j*xsize];
  ...
  { 
    ...
    // if both endpoints outside image domain, skip entierly (edge events)
    // else, start from the endpoint inside the image domain 
    //new: either select in-frame point OR interpolae x2,y2 in average z
    //new: lx will be averaged z middle 
    ...
    lxm = (int) floor((lx+lx2)/2);
    lym = (int) floor((ly+ly2)/2);
    lzm = (int) floor((lz+lz2)/2);

    //use midpoint if possible, resort to an endpoint, or skip
    if ( (lxm<=0.)||(lym<=0.)||(lzm<=0.)||
    (lxm>=xsize)||(lym>=ysize)||(lzm>=zsize))
    {
      //printf("ERROR: 3Dlsd3b: midpoint out of bounds");fflush(stdout);
      continue; 
      if ( (lx<=0.)||(ly<=0.)||(lz<=0.)||
      (lx>=xsize)||(ly>=ysize)||(lz>=zsize))
      {
        ...
        else {lxm=lx2; lym=ly2; lzm=lz2;}
      }
      else {lxm=lx; lym=ly; lzm=lz;}
    }
    lstheta = line_angle3(lx,ly,lz,lx2,ly2,lz2);    
    lxx=lxm; lyy=lym; lzz=lzm;
    //for(int radius=0; radius<=searchbox; radius++)
    for(lx=lxx-searchbox;lx<lxx+searchbox;lx++)
    for(ly=lyy-searchbox;ly<lyy+searchbox;ly++)
    for(lz=lzz-searchbox;lz<lzz+searchbox;lz++)
    {
      //if((abs(lx-lxx)==radius)||(abs(ly-lyy)==radius)||(abs(lz-lzz)==radius))
      //  continue;
      if ((lx<0) || (ly<0) || (lz<0) ||
          ...
          elimg[ lz + (lx + ly * xsize) * zsize ] != NOTDEF  )
      {
        ...
        {
          rect3_iter *pr;
          double tempx,tempy,tempidx,tempatan;
          tempidx=0;tempx=0;tempy=0;tempz=0;
          int prx,pry,prz;
          prx=0;pry=0;prz=0;
          for(pr=ri3_ini(&rec); !ri3_end(pr); ri3_inc(pr))
          {
            prx=(int)pr->x;pry=(int)pr->y;prz=(int)pr->z;
            if(((prx>=0) && (prx<xsize))
              &&((pry>=0) && (pry<ysize))
              &&((prz>=0) && (prz<zsize)))
            {
              return_value[prz+zsize*(prx+xsize*pry)]=(double)100.;//ls_count2;
              //if (tempidx==0){tempx=(double)prx; tempy=(double)pry;}
              //tempidx++;
              //if (tempidx==100){tempatan=atan2((double)pry - tempy,(double)prx - tempx);printf("atan %.2f",tempatan);fflush(stdout);}
            }
          ri3_del(pr);
          }
        }
        if (printreg==2)
        {
          for(int pr=0;pr<(int)reg_size;pr++)
          {
            if(((reg[pr].x>=0) && (reg[pr].x<xsize))
              &&((reg[pr].y>=0) && (reg[pr].y<ysize))
              &&((reg[pr].z>=0) && (reg[pr].z<zsize)))
            {
              return_value[(int)reg[pr].z+zsize*((int)reg[pr].x+xsize*(int)reg[pr].y)]=(double)1.;//ls_count2;
              //grads_az = angles->az->data[ (int)reg[pr].z + zsize*((int)reg[pr].x + (int)reg[pr].y * xsize) ]; 
              //grads_el = angles->el->data[ (int)reg[pr].z + zsize*((int)reg[pr].x + (int)reg[pr].y * xsize) ]; 
              //return_value[(int)reg[pr].z+zsize*((int)reg[pr].x+xsize*(int)reg[pr].y)]=(double)isaligned3ORTH(grads_az,grads_el,lstheta->az,lstheta->el,prec	);
            }
           }
