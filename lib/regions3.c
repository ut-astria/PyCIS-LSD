...
lib/regions3.c - region approximation, measurement, and improvement functions for the 3D case.
                         ...
                         image3_double modgrad, angles3 reg_angle, double prec, int orth )
{
  ...
  if(orth==1)
  {
    theta->az = reg_angle->az; theta->el = reg_angle->el;
    //reg_angle->az = theta->az;reg_angle->el = theta->el;
  }
                         ...
                         double prec, double p, struct rect3 * rec , int orth )
{
  ...
  rec->prec = prec;
  rec->p = p;
  rec->density=0;//must finish before can create iterator for density compute
  ...
  //UPDATE DENSITY 
  //double log_nfa_new = rect3_nfa(&rec,angles,0,0,0,0);
}
                         ...
                         image3_char used,double prec ,int NOUT)
{
  ...
  *reg_size = 1;
  ...
  int zsize = (int)used->zsize;
  used->data[z+(x+y*xsize)*zsize] = USED;
  double temp_az,temp_el;
      temp_az = angles->az->data[ z + zsize*(x + y * xsize) ]; 
      temp_el = angles->el->data[ z + zsize*(x + y * xsize) ]; 
...
;
  ...
  for(i=0; i<*reg_size; i++)
    ...
    for(xx=reg[i].x-1; xx<=reg[i].x+1; xx++)
    {
      ...
      {
          /* add point */
          //if(!isaligned3_sign(grads_az,grads_el,temp_az,temp_el,prec/2.))
          //  continue; 
          temp_az=grads_az; temp_el=grads_el;
          used->data[zz+(xx+yy*xsize)*zsize] = USED;
          reg[*reg_size].x = xx;
          reg[*reg_size].y = yy;
          reg[*reg_size].z = zz;
                                 ...
                                 struct rect3 * rec,double log_nfa,int orth)
{
  ...
  if (rec->length<=max2(rec->width1,rec->width2)) return -101.;
                                 ...
                                 double density_th , int orth)
{
  ...
  double log_nfa_new = rect3_nfa(rec,angles,0,0,0,0);
  density = rec->density;
  //log_nfa_new = rect3_nfaORTH(&rec,angles,0,0,0,0,0);
  ...
  while( density < density_th )
  {
    ...
    if (rec->length<=max2(rec->width1,rec->width2)) return FALSE;
    /* re-compute region points density */
    log_nfa_new = rect3_nfa(rec,angles,0,0,0,0);
    density = rec->density;
    //density = (double) *reg_size /
    //                   ( dist3(rec->x1,rec->y1,rec->z1,rec->x2,rec->y2,rec->z2) 
    //                   * rec->width1 * rec->width2 );
